...
export const listarTodos = async (req, res) => {
  try {
    const { categoria, endereco_loja, modo } = req.query;
    ...
    if (modo === "autocomplete") {
      ...
      if (!busca) {
        return res.status(400).json({
          status: 400,
          success: false,
          message: "É necessário digitar um título",
        });
      }

      // Filtro para buscar por título de desconto
      const filtrosAutocomplete = {
        TITULO: { contains: busca, mode: "insensitive" },
      };

      const descontos = await descontosModel.listarTodos(filtrosAutocomplete);

      if (!descontos || descontos.length === 0) {
        return res.status(404).json({
          status: 404,
          success: false,
          message: "Não foi encontrado nenhum desconto com esse título",
        });
      }
...
export const criar = async (req, res) => {
  try {
    ...
    const categoriaValida = categoriasPermitidas
      .map((c) => c.toLowerCase())
      .includes(dado.CATEGORIA.toLowerCase());
    ...
    if (dado.TITULO && dado.TITULO !== descontoExiste.TITULO) {
      ...
      if (tituloExiste && tituloExiste.ID_DESCONTO !== id) {
        return res.status(409).json({
          ...
          success: false,
          message:
            "Não é possível atualizar um desconto com um título já existente",
          error: "DUPLICATE_TITLE",
          suggestion: [
            "Tente usar um título diferente",
            "Verifique os descontos já cadastrados",
